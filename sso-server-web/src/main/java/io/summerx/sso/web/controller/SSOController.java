...
public class SSOController {
     ...
     * 是否总是跳转到Welcome的页面
     */
    @Value("${sso.welcome.redirect}")
    private boolean alwaysRedirect = false;

    /**
     * Welcome地址
     */
    @Value("${sso.welcome.url}")
    ...
    /**
     * 核实用户是否已经登录，如果已经登录则颁发一个ST凭证，用户可以拿着ST去访问应用
     */
    ...
    private ModelAndView redirectIfLogin(UserAuthorization authorization, String retUrl) {
        ModelAndView mv;
        if (!alwaysRedirect && !StringUtils.isEmpty(retUrl) && isInWhitelist(retUrl)) {
            mv = new ModelAndView("redirect:" + retUrl);
        } else {
            // 跳转到指定页面（如：首页）
            mv = new ModelAndView("redirect:" + welcomeUrl);
        }
        return mv;
    }
     ...
     * @return
     */
    protected boolean isInWhitelist(String retUrl) {
        // TODO 增加白名单功能，只有白名单中的Url才可以走SSO通道。
        return true;
    }
}
